...
grep -q "anon_root=" "$conf" && sed -i -e "#anon_root=#s#^[#]##g;#anon_root=#s#\=.*#=/var/ftp#g" "$conf" || echo "anon_root=/var/ftp" >> "$conf" #设置匿名用户根目录
grep -q "no_anon_password=" "$conf" && sed -i -e "/no_anon_password=/s/^[#]//g;/no_anon_password=/s/\=.*/=YES/g" "$conf" || echo "no_anon_password=YES" >> "$conf" #允许无密码登录
...
if [[ $(grep -c "^$user:" /etc/passwd) -eq 0 ]];then
		useradd -d /home/cuc/ftp/$user -s /bin/false $user
...
sed -i -e "pam_service_name=ftp" "$conf"
...
grep -q "vsftpd:192.168.56.113" /etc/hosts.allow || echo "vsftpd:192.168.56.113" >> /etc/hosts.allow
